...
默认还会写入 WebHTV C16 合同所需的 `tmdb` 结构化字段，包含身份、语言、抓取时间、能力组和 TMDB 原始详情。APP 可直接复用其中的核心信息、演职员、图片、外部 ID、视频和推荐，避免再次访问 TMDB；电视剧会按当前季嵌入季摘要、集列表、季演职员和季图片。若只需旧版 `vod_*` 平铺字段，可将 `include_tmdb_payload` 设为 `false`。
